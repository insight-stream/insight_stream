...
ENCODING_FORMAT = os.getenv("ENCODING_FORMAT", "float")
TIKTOKEN_MODEL = os.getenv("TIKTOKEN_MODEL", "Salesforce/SFR-Embedding-Mistral")
...
def ask(bot_id: str, question: str) -> Optional[Dict[str, any]]:
...
def _add_documents(
    index_id: str, documents: List[Document], file_path: str
) -> List[Document]:
    ...
    qdrant = Qdrant(
        client=clientQdrant, collection_name=index_id, embeddings=embeddings
    )

    docs_for_qdrant = []

    prompt_kw = ChatPromptTemplate.from_template(
        "Выбери абревиатуры из текста {chunk} для которых есть расшифровка значения, "
        ...
        + "только специфические для данного стандарта. В ответ дай только список уникальных абревиатур, БЕЗ ПОВТОРЕНИЙ. "
        ...
        new_doc = Document(
            ...
            metadata={
                ...
                "url": urllib.parse.quote(
                    f"{SERVER_NAME}/documents/{os.path.basename(file_path)}", safe=":/"
                ),
